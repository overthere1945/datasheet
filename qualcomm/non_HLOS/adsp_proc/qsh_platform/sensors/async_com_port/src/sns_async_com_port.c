...
/*=============================================================================
  ...
  ===========================================================================*/
#ifndef SNS_ASCP_THREAD_NAME
#define SNS_ASCP_THREAD_NAME "sns_sig_ascp"
#endif
#ifndef SNS_ASCP_THREAD_PRIO
...
static const sns_sensor_uid
    sensor_uid SNS_SECTION(".rodata.sns") = ASYNC_COM_PORT_SUID;

/*=============================================================================
  Extern functions
  ===========================================================================*/

extern sns_rc sns_async_com_port_inst_generate_event(
...
SNS_SECTION(".text.sns")
static void sns_async_com_port_signal_cb(sns_sensor *const this,
                                         sns_signal_handle *sig_handle,
                                         void *args)
{
  UNUSED_VAR(args);
  sns_sensor_instance *inst;
  sns_async_com_port_state *state =
      (sns_async_com_port_state *)this->state->state;

  // Check the signal to see if it's for async_com_port
  if(sig_handle == state->sig_handle)
  {
    // Handle the async_com_port signal
    for(inst = this->cb->get_sensor_instance(this, true); NULL != inst;
...
static void sns_async_com_port_publish_attributes(sns_sensor *const this)
{
  {
  ...
  {
    char const proto_files[] = "sns_async_com_port.proto";
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
    value.str.funcs.encode = pb_encode_string_cb;
    value.str.arg =
...
static sns_rc sns_async_com_port_init(sns_sensor *const this)
{
  ...
  sns_signal_attr_set(&signal_attr, this, &sns_async_com_port_signal_cb, NULL);

  rc = sns_signal_register(&signal_attr, &state->sig_handle);

  if(SNS_RC_SUCCESS != rc)
  {
    SNS_PRINTF(ERROR, this,
               "sns_async_com_port_init - sns_signal_register() failed");
  }
  else
  {
...
static sns_rc sns_async_com_port_deinit(sns_sensor *const this)
{
  sns_rc rc = SNS_RC_SUCCESS;
  sns_async_com_port_state *state =
      (sns_async_com_port_state *)this->state->state;
  rc = sns_signal_unregister(&state->sig_handle);
  return rc;
}
                                      ...
                                      sns_request const *new_req, bool remove)
{
  if(remove)
  {
    sns_sensor_instance *inst = sns_sensor_util_find_instance(
        this, curr_req, sns_async_com_port_get_sensor_uid(this));
    if(NULL != inst)
    {
      inst->cb->remove_client_request(inst, curr_req);
      if(NULL == inst->cb->get_client_request(
                     inst, sns_async_com_port_get_sensor_uid(this), true))
      {
  ...
  else
  {
    ...
    {
      sns_sensor_instance *new_async_com_port_inst = this->cb->create_instance(
          this, sizeof(sns_async_com_port_instance_state));
      // New request -- must be a COM_PORT_CONFIG
      if(NULL != new_async_com_port_inst)
      {
        new_async_com_port_inst->cb->add_client_request(new_async_com_port_inst,
                                                        new_req);
        if(SNS_ASYNC_COM_PORT_MSGID_SNS_ASYNC_COM_PORT_CONFIG ==
               new_req->message_id &&
           SNS_RC_SUCCESS == this->instance_api->set_client_config(
                                 new_async_com_port_inst, new_req))
        {
          if(!pb_send_event(
                 new_async_com_port_inst, NULL, NULL, sns_get_system_time(),
        ...
        {
          SNS_PRINTF(ERROR, this, "set client config failed. msg_id %u",
                     new_req->message_id);
          sns_async_com_port_send_error(new_async_com_port_inst,
                                        SNS_RC_INVALID_VALUE);
          new_async_com_port_inst->cb->remove_client_request(
              new_async_com_port_inst, new_req);
    ...
    else
    {
      ...
      {
        SNS_PRINTF(ERROR, this,
                   "Trying to send a COM_PORT_CONFIG on an already configured "
                   "stream.");
...
SNS_SECTION(".text.sns")
static sns_rc sns_async_com_port_inst_notify_event(sns_sensor *const this)
{
  UNUSED_VAR(this);
  return SNS_RC_SUCCESS;
}

const sns_sensor_api async_com_port_api SNS_SECTION(".rodata.sns") = {
