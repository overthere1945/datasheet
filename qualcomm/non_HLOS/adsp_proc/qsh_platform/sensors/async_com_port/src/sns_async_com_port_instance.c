...
static void sns_async_com_port_callback(void *port_handle, uint8_t num_vectors,
                                        ...
                                        void *user_args)
{
  ...
  if(NULL != state)
  {
    ...
    state->rw_err = rw_err;
    sns_signal_set(state->sig_handle);
  }
}

SNS_SECTION(".text.sns")
static void extract_vectors(sns_port_vector *vector, void *user_args)
{
  sns_async_com_port_queued_transaction *extracted_vectors =
  ...
  extracted_vectors->vectors[curr_index].buffer =
      sns_malloc(SNS_HEAP_ISLAND, vector->bytes);
  if(NULL == extracted_vectors->vectors[curr_index].buffer)
  {
    SNS_ISLAND_EXIT();
    extracted_vectors->vectors[curr_index].buffer =
        sns_malloc(SNS_HEAP_MAIN, vector->bytes);
  }
  if(extracted_vectors->vectors[curr_index].is_write)
  {
    sns_memscpy(extracted_vectors->vectors[curr_index].buffer, vector->bytes,
                vector->buffer, vector->bytes);
  }
  extracted_vectors->num_vectors += 1;
}

SNS_SECTION(".text.sns")
static bool sns_async_com_port_pb_req_dec_cb(pb_istream_t *stream,
                                             const pb_field_t *fields,
                                             void **args)
{
  UNUSED_VAR(fields);
  ascp_pb_dec_cb_args *dec_args = *args;
  return sns_ascp_for_each_vector_do(stream, extract_vectors,
                                     &dec_args->extracted_vectors);
}
...
{
  pb_istream_t stream = pb_istream_from_buffer(client_request->request,
  ...
  pb_simple_cb_arg arg =
      (pb_simple_cb_arg){.decoded_struct = &decoded_struct,
                         .fields = sns_async_com_port_vector_rw_fields};
  temp.payload.funcs.decode = pb_decode_simple_cb;
  temp.payload.arg = &arg;
  pb_decode_noinit(&stream, sns_std_request_fields, &temp);
  return decoded_struct.num_vectors;
}

SNS_SECTION(".text.sns")
void sns_async_com_port_get_vectors_from_req(sns_request const *client_request,
                                             ...
                                             sns_sensor_instance *const this)
{
  ...
  *vectors =
      sns_malloc(SNS_HEAP_ISLAND, sizeof(sns_port_vector) * vectors_count);
  if(NULL == *vectors)
  {
    SNS_ISLAND_EXIT();
    *vectors =
        sns_malloc(SNS_HEAP_MAIN, sizeof(sns_port_vector) * vectors_count);
  }
  ...
  sns_std_request temp;
  sns_memzero(&temp, sizeof(sns_std_request));
  ascp_pb_dec_cb_args args =
      (ascp_pb_dec_cb_args){.instance = this,
...
SNS_SECTION(".text.sns")
static uint32_t sns_async_com_port_get_pending_transactions(
    sns_async_com_port_instance_state *state)
{
                                     ...
                                     sns_sensor_instance *const this)
{
  ...
  sns_async_com_port_get_vectors_from_req(client_request, &vectors,
                                          &num_vectors, this);

  sns_async_com_port_queued_transaction *tx = sns_malloc(
      SNS_HEAP_ISLAND, sizeof(sns_async_com_port_queued_transaction));

  if(NULL == tx)
  {
    SNS_ISLAND_EXIT();
    tx = sns_malloc(SNS_HEAP_MAIN,
                    sizeof(sns_async_com_port_queued_transaction));
  }
  SNS_ASSERT(NULL != tx);
...
SNS_SECTION(".text.sns")
static sns_rc sns_async_com_port_dequeue_transaction(
    ...
    sns_async_com_port_instance_state *state)
{
      ...
      curr_pending_tx = sns_isafe_list_iter_advance(&iter))
  {
    if(tx == sns_isafe_list_item_get_data(curr_pending_tx))
    {
      ...
      // debug
      sns_memscpy(&state->pre_vector, sizeof(state->pre_vector),
                  &tx->vectors[0], sizeof(tx->vectors[0]));
...
SNS_SECTION(".text.sns")
static void sns_async_com_port_send_error(sns_sensor_instance *const this,
                                          sns_rc error)
{
  uint32_t i;
  ...
  {
    sns_async_com_port_error async_com_port_event =
        sns_async_com_port_error_init_default;
    ...
    pb_send_event(this, sns_async_com_port_error_fields, &async_com_port_event,
                  sns_get_system_time(),
                  SNS_ASYNC_COM_PORT_MSGID_SNS_ASYNC_COM_PORT_ERROR, NULL);
  }
}
...
sns_async_com_port_start_next_transaction(sns_sensor_instance *const this)
{
  ...
  if(NULL != tx)
  {
    ...
    else
    {
      ...
      for(i = 0; i < state->current_tx->num_vectors; i++)
      {
        if(!sns_island_is_island_ptr(
               (intptr_t)state->current_tx->vectors[i].buffer))
        {
          SNS_ISLAND_EXIT();
        }
    ...
    {
      ret_val = sns_async_port_api.sync_com_port->sns_scp_update_bus_power(
          state->priv_handle, true);
      if(SNS_RC_SUCCESS != ret_val)
      {
        SNS_INST_PRINTF(ERROR, this, "scp_update_bus_power failed with %u",
                        ret_val);
      }
      state->rw_err = sns_async_port_api.sns_ascp_register_rw(
          state->priv_handle, tx->num_vectors, tx->vectors, false);
    }
  ...
  if(NULL != tx)
  {
    ...
    if(SNS_RC_SUCCESS == state->rw_err)
    {
      uint32_t buffer_size =
          sns_ascp_estimate_pb_buffer_size(tx->vectors, tx->num_vectors, false);
      uint32_t enc_len = 0;
      sns_sensor_event *event_out =
          event_service->api->alloc_event(event_service, this, buffer_size);
      if(NULL != event_out)
      {
        if(sns_ascp_create_encoded_vectors_buffer(
               tx->vectors, tx->num_vectors, false, (uint8_t *)event_out->event,
               event_out->event_len, &enc_len))
        {
          event_out->message_id =
              SNS_ASYNC_COM_PORT_MSGID_SNS_ASYNC_COM_PORT_VECTOR_RW;
          event_out->timestamp = sns_get_system_time();
          event_out->event_len = enc_len;
          event_service->api->publish_event(event_service, this, event_out,
                                            NULL);
        }
        else
        {
    ...
    sns_async_port_api.sync_com_port->sns_scp_update_bus_power(
        state->priv_handle, false);
...
SNS_SECTION(".text.sns")
static sns_rc sns_async_com_port_inst_init(sns_sensor_instance *const this,
                                           sns_sensor_state const *sensor_state)
{
  UNUSED_VAR(sensor_state);
  sns_async_com_port_instance_state *state =
      (sns_async_com_port_instance_state *)this->state->state;
  sns_async_com_port_state *ascp_sensor_state =
      (sns_async_com_port_state *)sensor_state->state;
  sns_memzero(state, sizeof(sns_async_com_port_instance_state));
  sns_isafe_list_init(&state->pending_transactions);

  state->sig_handle = ascp_sensor_state->sig_handle;
  state->current_tx = NULL;
  return SNS_RC_SUCCESS;
}

SNS_SECTION(".text.sns")
static sns_rc sns_async_com_port_inst_deinit(sns_sensor_instance *const this)
{
  sns_async_com_port_instance_state *state =
      (sns_async_com_port_instance_state *)this->state->state;
  sns_isafe_list_iter iter;
  ...
  {
    sns_async_port_api.sync_com_port->sns_scp_deregister_com_port(
...
SNS_SECTION(".text.sns")
sns_bus_type sns_ascp_map_bus_type(sns_async_com_port_bus_type ascp_bus_type)
{
  ...
  case SNS_ASYNC_COM_PORT_BUS_TYPE_SPI:
    return SNS_BUS_SPI;
  case SNS_ASYNC_COM_PORT_BUS_TYPE_I3C_SDR:
    return SNS_BUS_I3C_SDR;
  case SNS_ASYNC_COM_PORT_BUS_TYPE_I3C_HDR_DDR:
    return SNS_BUS_I3C_HDR_DDR;
  case SNS_ASYNC_COM_PORT_BUS_TYPE_I3C_I2C_LEGACY:
    return SNS_BUS_I3C_I2C_LEGACY;
  default:
    return SNS_BUS_MIN;
  }
}
...
SNS_SECTION(".text.sns")
sns_qup_type sns_ascp_map_qup_type(sns_acp_qup_type qup_type)
{
  switch(qup_type)
  {
  case SNS_ACP_SSC_QUP:
    return SNS_SSC_QUP;
  case SNS_ACP_TOP_QUP:
    return SNS_TOP_QUP;
  default:
    return SNS_SSC_QUP;
  }
                                          ...
                                          sns_request const *client_request)
{
  ...
  if(SNS_ASYNC_COM_PORT_MSGID_SNS_ASYNC_COM_PORT_CONFIG ==
     client_request->message_id)
  {
    ...
    if(pb_decode_request(client_request, sns_async_com_port_config_fields, NULL,
                         &async_port_config))
    {
      port_config.bus_instance = async_port_config.bus_instance;
      port_config.qup_type = sns_ascp_map_qup_type(
          async_port_config.has_qup_type ? async_port_config.qup_type
                                         : SNS_ACP_SSC_QUP);
      port_config.qup_instance = async_port_config.has_qup_instance
                                     ? async_port_config.qup_instance
                                     : 0;
      port_config.manufacturer_id = async_port_config.has_manufacturer_id
                                     ? async_port_config.manufacturer_id
                                     : 0;
      port_config.provisional_id = async_port_config.has_provisional_id
                                        ? async_port_config.provisional_id
                                        : 0;
      ...
      port_config.reg_addr_type =
          sns_ascp_map_reg_addr_type(async_port_config.reg_addr_type);
      port_config.slave_control = async_port_config.slave_control;
      port_config.sub_sys_type = 0;
      sns_memscpy(&state->config, sizeof(sns_com_port_config), &port_config,
                  sizeof(sns_com_port_config));

      ret_val = sns_async_port_api.sync_com_port->sns_scp_register_com_port(
          &state->config, (sns_sync_com_port_handle **)&state->priv_handle);
      if(SNS_RC_SUCCESS == ret_val)
      {
        sns_async_port_api.sns_ascp_register_callback(
            state->priv_handle, sns_async_com_port_callback, state);
        SNS_INST_PRINTF(LOW, this,
                        "opening port for async inst:%d type:%d addr:%d",
                        port_config.bus_instance, port_config.bus_type,
                        port_config.slave_control);
        ret_val =
            sns_async_port_api.sync_com_port->sns_scp_open(state->priv_handle);
        if(SNS_RC_SUCCESS == ret_val)
        {
          ret_val = sns_async_port_api.sync_com_port->sns_scp_update_bus_power(
              state->priv_handle, false);
          if(SNS_RC_SUCCESS != ret_val)
          {
            SNS_INST_PRINTF(ERROR, this, "sns_scp_update_bus_power Failed %d",
                            ret_val);
          }
        }
        else
      ...
      {
        SNS_INST_PRINTF(ERROR, this, "sns_scp_register_com_port Failed %d",
                        ret_val);
      }
    }
    else
    {
      SNS_INST_PRINTF(ERROR, this,
                      "Failed to decode "
                      "SNS_ASYNC_COM_PORT_MSGID_SNS_ASYNC_COM_PORT_CONFIG req");
    }
    if(SNS_RC_SUCCESS != ret_val)
    {
  ...
  else if(SNS_ASYNC_COM_PORT_MSGID_SNS_ASYNC_COM_PORT_VECTOR_RW ==
          client_request->message_id)
  {
    sns_async_com_port_queue_transaction(state, client_request, this);
    while(NULL == state->current_tx &&
          0 < sns_async_com_port_get_pending_transactions(state))
    {
...
SNS_SECTION(".text.sns")
sns_rc sns_async_com_port_inst_generate_event(
    sns_sensor_instance *const async_com_port_inst)
{
  sns_async_com_port_instance_state *state =
  ...
  if(state->last_rw_event_timestamp != state->last_rw_timestamp)
  {
    ...
    sns_async_com_port_finish_tx_and_send(state, event_service,
                                          async_com_port_inst);
...
const sns_sensor_instance_api
    sns_async_com_port_instance_api SNS_SECTION(".rodata.sns") = {
