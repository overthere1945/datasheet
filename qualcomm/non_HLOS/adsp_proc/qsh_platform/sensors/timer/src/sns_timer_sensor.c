...
#include "sns_thread_utilization.h"
...
/*=============================================================================
  ...
  ===========================================================================*/
#define SNS_TIMER_EVENT_SIGNAL 0x1

/* Enable for extensive & costly debugging messages */
//#define SNS_TIMER_ENABLE_DEBUG

/* Periodic Timers will not be created for intervals smaller than 0.25ms */
#define TIMER_MIN_INTERVAL_NS 250000
...
/*=============================================================================
  Globals
  ===========================================================================*/
static const sns_std_request default_sns_std_request
    SNS_SECTION(".rodata.sns") = sns_std_request_init_default;
static const sns_timer_sensor_config default_sns_timer_sensor_config
    SNS_SECTION(".rodata.sns") = sns_timer_sensor_config_init_default;
...
static sns_isafe_list_item *sns_timer_get_list_head(sns_isafe_list *list)
{
 ...
 */
SNS_SECTION(".text.sns")
static sns_time sns_timer_select_timer_start(int64_t target_start,
 ...
 *
 * This saves information about the current time into the "read only" sensor
 * buffer, and signals the timer sensor via the OS signal.
 *
 * @param[i] args   Pointer to the timer sensor "read-only" buffer
 */
SNS_SECTION(".text.sns")
static void sns_timer_sensor_callback(sns_timer_cb_func_arg args)
{
  ...
  SNS_UPRINTF(LOW, sns_fw_printf, "Timer callback now %x", (uint32_t)now);
#endif
  sns_osa_thread_sigs_set(ro_buffer->thread, ro_buffer->signal);
}
 ...
 *
 * Will start the system timer based on the first timer in the list of
 * registered timers. If the list is empty, it will stop the system timer.
 *
 * @param[i] state    Timer sensor state
 */
SNS_SECTION(".text.sns")
static void sns_timer_sensor_arm_system_timer(sns_timer_state *state)
{
  ...
  if(NULL != iter_item)
  {
    sns_sensor_instance *registered_inst =
        sns_isafe_list_item_get_data(iter_item);
    sns_timer_instance_state *registered_entry =
        (sns_timer_instance_state *)(registered_inst->state->state);

    sns_timer_start_absolute(state->system_timer,
                             registered_entry->trigger_time);
#ifdef SNS_TIMER_ENABLE_DEBUG
    SNS_PRINTF(LOW, sns_fw_printf, "Set timer to %x",
 ...
 * There are two lists:
 * Registered timers:
 *   This list is sorted first by next timeout, and then by priority. It does
 * not contain "dry_run" (aka virtual) timers, whether periodic or not.
 *
 * Periodic timers:
 *   This list contains only periodic timers. It is sorted by period, and then
 * by priority. It may contain dry_run (virtual) periodic timers.
 *
 * @param[i] state          Timer sensor state
 * @param[i] inst           Instance state (the new timer to add to the list)
 * @param[i] rearm_timer_if_needed  Will rearm the system timer if the list head
 * changes
 *
 * @return SNS_RC_SUCCESS
 */
SNS_SECTION(".text.sns")
static sns_rc sns_timer_sensor_record_entry(sns_timer_state *state,
                                            ...
                                            bool rearm_timer_if_needed)
{
  ...
  sns_isafe_list_item *iter_item;
  sns_timer_instance_state *time_entry =
    ...
    while(NULL != iter_item)
    {
      ...
      if(time_entry->trigger_time <= sorted_trigger_time)
      {
        while(time_entry->trigger_time == registered_entry->trigger_time &&
              time_entry->client_config.priority <
                  registered_entry->client_config.priority &&
     ...
     !sns_isafe_list_item_present(&time_entry->periodic_entry))
  {
    ...
    while(NULL != iter_item)
    {
      ...
      sns_timer_instance_state *registered_entry =
          (sns_timer_instance_state *)(registered_inst->state->state);
      sns_time sorted_timeout_period =
  ...
  if(rearm_timer_if_needed && !time_entry->client_config.is_dry_run)
  {
 ...
 * @param[i] rearm_timer_if_needed  Will rearm the system timer if the list head
 * changes
 * @param[i] delete_from_periodic_list  Whether to remove the timer from the
 * list of periodic timers or not
 */
SNS_SECTION(".text.sns")
static void sns_timer_sensor_delete_entry(sns_timer_state *state,
                                          sns_sensor_instance *inst,
                                          bool rearm_timer_if_needed,
                                          bool delete_from_periodic_list)
{
  ...
  {
    inst_head = sns_isafe_list_item_get_data(
        sns_timer_get_list_head(&state->registered_timers));
...
#ifdef SNS_TIMER_ENABLE_DEBUG
  SNS_PRINTF(LOW, sns_fw_printf, "Deleted timer: %x", (intptr_t)inst);
#endif
}
 ...
 *
 * Virtual timers do not expire, so their trigger_times are updated only when
 ...
 */
SNS_SECTION(".text.sns")
static void sns_timer_update_dry_run_timers(sns_timer_state *state,
...
#ifdef SNS_TIMER_ENABLE_DEBUG
      SNS_PRINTF(
          LOW, sns_fw_printf, "updating dryrun timer to: %x. sys_time %x",
          (uint32_t)registered_entry->trigger_time, (uint32_t)system_time);
#endif
    }
    iter_item = sns_isafe_list_iter_advance(&iter);
  }
 ...
 * Periodic timers are advanced to the appropriate spot in the registered timer
 * list. The list of periodic timers is not changed. This function assumes that
 * all events associated with the expired timers have already been sent.
 *
 ...
 */
SNS_SECTION(".text.sns")
static sns_rc sns_timer_sensor_handle_expired(sns_sensor *this,
                                              sns_time last_timeout)
{
  ...
  while(NULL !=
        (list_head = sns_timer_get_list_head(&state->registered_timers)))
  {
    sns_sensor_instance *inst =
        (sns_sensor_instance *)sns_isafe_list_item_get_data(list_head);
    sns_timer_instance_state *entry =
        (sns_timer_instance_state *)inst->state->state;
    if(entry->trigger_time <= last_timeout)
    {
      ...
      {
        sns_sensor_util_remove_all_client_req(
            inst, sns_timer_sensor_get_sensor_uid(this));
...
SNS_SECTION(".text.sns")
static bool send_timer_sensor_event(sns_sensor_instance *instance,
                                    ...
                                    sns_time timestamp)
{
  ...
  size_t encoded_len = MAX_UINT64_ENCODED_SIZE * 2 + TAG_SIZE * 2;
  _Static_assert(MAX_UINT64_ENCODED_SIZE * 2 + TAG_SIZE * 2 ==
                     sns_timer_sensor_event_size,
                 "Update timer encoded len");
  ...
  if(event == NULL)
  {
    SNS_INST_PRINTF(ERROR, instance, "Unable to allocate timer sensor event.");
  }
  else
  {
    ...
    // TAG
    encoded_event[stream_idx++] =
        sns_timer_sensor_event_requested_timeout_time_tag << 3 | PB_WT_VARINT;
    // Payload
    sns_pb_encode_varint(&encoded_event[stream_idx], MAX_UINT64_ENCODED_SIZE,
                         timer_event->requested_timeout_time, &bytes_written);
    stream_idx += bytes_written;

    // TAG
    encoded_event[stream_idx++] =
        sns_timer_sensor_event_timeout_time_tag << 3 | PB_WT_VARINT;
    // Payload
    sns_pb_encode_varint(&encoded_event[stream_idx], MAX_UINT64_ENCODED_SIZE,
                         timer_event->timeout_time, &bytes_written);
    stream_idx += bytes_written;

    event->event_len = stream_idx;
    event->message_id = SNS_TIMER_MSGID_SNS_TIMER_SENSOR_EVENT;
                              ...
                              sns_time system_time)
{
  ...
  if(!send_timer_sensor_event(timer_inst, &timer_event, system_time))
  {
 ...
 */
SNS_SECTION(".text.sns")
static void sns_timer_process_signal_events(sns_sensor *this, uint32_t signal,
                                            void const *args)
{
  UNUSED_VAR(signal);
  UNUSED_VAR(args);
  ...
  SNS_PRINTF(LOW, sns_fw_printf,
             "Timer process sig events last TO state:%x ro:%x now:%x",
             (uint32_t)state->last_timeout_id_processed,
             (uint32_t)state->ro_buffer.timeout_event.last_timeout_id,
             (uint32_t)now);
#endif
  if(state->last_timeout_id_processed !=
     state->ro_buffer.timeout_event.last_timeout_id)
  {
    sns_time timeout_limit = now + min_timeout_ticks;
    sns_isafe_list_item *iter_item;

    state->last_timeout_id_processed =
        state->ro_buffer.timeout_event.last_timeout_id;
 ...
 */
static void sns_timer_sensor_publish_attributes(sns_sensor *const this)
{
  {
    ...
    value.str.arg =
        &((pb_buffer_arg){.buf = name_type, .buf_len = sizeof(name_type)});
  ...
  {
    char const vendor[] = "QTI_internal";
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
    value.str.funcs.encode = pb_encode_string_cb;
    value.str.arg =
 ...
 */
SNS_SECTION(".text.sns")
static bool sns_timer_get_decoded_req(sns_request const *in_request,
                                      ...
                                      sns_timer_sensor_config *decoded_payload)
{
  ...
  decoded_request->payload =
      (struct pb_callback_s){.funcs.decode = &pb_decode_simple_cb, .arg = &arg};
 ...
 */
SNS_SECTION(".text.sns")
static void sns_timer_find_aperiodic_best_fit(
    sns_timer_state *state, sns_timer_sensor_config const *const req_config,
    sns_timer_sensor_reg_event *selected_config)
{
  ...
  while(NULL != iter_item)
  {
    ...
    SNS_PRINTF(LOW, sns_fw_printf, "finding best fit expy %x, period %x",
               (uint32_t)expiry_under_test, (uint32_t)period_under_test);
    SNS_PRINTF(LOW, sns_fw_printf, "floor %x ceil %x", floor, ceiling);
#endif
    if(ceiling <= floor)
    {
 ...
 */
SNS_SECTION(".text.sns")
static void sns_timer_find_periodic_best_fit(
    sns_timer_state *state, sns_timer_sensor_config const *const req_config,
    sns_timer_sensor_reg_event *selected_config)
{
  sns_isafe_list_iter iter;
...
#ifdef SNS_TIMER_ENABLE_DEBUG
  SNS_PRINTF(LOW, sns_fw_printf,
             "Finding periodic best fit for: shortest_period %x longest_period "
             "%x req_period %x",
             (uint32_t)shortest_period, (uint32_t)longest_period,
             (uint32_t)target_period);
  SNS_PRINTF(LOW, sns_fw_printf,
             "Finding periodic best fit for: earliest_start %x latest_start %x "
             "target_start %x",
             (uint32_t)earliest_start, (uint32_t)latest_start,
  ...
  while(NULL != iter_item && selected_period == 0)
  {
...
#ifdef SNS_TIMER_ENABLE_DEBUG
      SNS_PRINTF(LOW, sns_fw_printf,
                 "No fit step 1. shortest > current timeout");
#endif
      break;
    }
    if(longest_period > (int64_t)cur_timer->timeout_period)
    {
      ...
      selected_start_time = sns_timer_select_timer_start(
          target_start, cur_inst_state->trigger_time, selected_period);
#ifdef SNS_TIMER_ENABLE_DEBUG
      SNS_PRINTF(LOW, sns_fw_printf,
                 "fit in step 1: selected_period %x cur trigger time %x",
    ...
    else
    {
      ...
      do
      {
        ...
        {
          if(llabs(period_under_test - target_period) <
             llabs(selected_period - target_period))
          {
  ...
  while(NULL != iter_item && selected_period == 0)
  {
    ...
    floor_of_num_periods_after = longest_period / period_under_test;
    ...
    {
      selected_period = (target_period / period_under_test) * period_under_test;
#ifdef SNS_TIMER_ENABLE_DEBUG
      SNS_PRINTF(LOW, sns_fw_printf, "fit step 2. selected_period %x",
                 (uint32_t)selected_period);
#endif
      ...
      {
        selected_start_time = sns_timer_select_timer_start(
            target_start, cur_inst_state->trigger_time, period_under_test);
#ifdef SNS_TIMER_ENABLE_DEBUG
        SNS_PRINTF(LOW, sns_fw_printf,
      ...
      {
#ifdef SNS_TIMER_ENABLE_DEBUG
        SNS_PRINTF(LOW, sns_fw_printf, "fit step 2 start no fit %x > %x",
  ...
  // Step 3: No matching periodic timers. At least try to match up with some
  // existing timer
  if(selected_period == 0)
  {
    ...
    while(NULL != iter_item)
    {
      ...
      {
        selected_start_time = sns_timer_select_timer_start(
            target_start, start_under_test, selected_period);
        break;
      }
      iter_item = sns_isafe_list_iter_return(&iter);
    }
                        ...
                        sns_timer_sensor_reg_event *selected_config)
{
  ...
  SNS_PRINTF(
      ...
      req_config->has_start_config, req_config->has_timeout_config,
      req_config->is_periodic);
#endif
  ...
  *selected_config =
  ...
  {
    selected_config->is_dry_run = false;
  }
  ...
  {
    selected_config->priority = SNS_TIMER_PRIORITY_OTHER;
  }
...
SNS_SECTION(".text.sns") static void timer_task(void *args)
{
  sns_sensor *this = (sns_sensor *)args;
  sns_timer_state *state = (sns_timer_state *)this->state->state;
  sns_osa_thread_sigs sigs;
  const uint32_t timer_task_sig =
      (SNS_TIMER_EVENT_SIGNAL | SNS_OSA_THREAD_EXIT_SIG);
  struct sns_watchdog_voter *wd_voter =
      sns_osa_thread_get_wd_voter(state->ro_buffer.thread);
  sns_osa_thread_active(wd_voter);

  sns_thread_utilization_compute(state->ro_buffer.thread, START_COMPUTE);

  for(;;)
  {
    sigs = 0;
    sns_thread_utilization_compute(state->ro_buffer.thread, STOP_COMPUTE);

    sns_osa_thread_sigs_wait_wrapper(timer_task_sig, &sigs,
                                     state->ro_buffer.thread);
    sns_es_low_mem_block();
    sns_thread_utilization_compute(state->ro_buffer.thread, START_COMPUTE);

    if(sigs & SNS_TIMER_EVENT_SIGNAL)
    {
      sns_osa_lock_acquire(((sns_fw_sensor *)this)->library->library_lock);
      sns_timer_process_signal_events(this, sigs, NULL);
      sns_osa_lock_release(((sns_fw_sensor *)this)->library->library_lock);
    }
    if(sigs & SNS_OSA_THREAD_EXIT_SIG)
    {
      SNS_PRINTF(HIGH, this, "Thread Exit");
      break;
    }
  }
}

static sns_rc sns_timer_sensor_init(sns_sensor *const this)
{
  ...
  state->ro_buffer.signal = SNS_TIMER_EVENT_SIGNAL;
  rc = sns_osa_thread_create_v2(SNS_TIMER_THREAD, (void *)&timer_task, this,
                                &state->ro_buffer.thread);
  ...
  if(SNS_RC_SUCCESS == rc)
  {
    ...
    // Create a virtual 1ms periodic timer for other timers to sync to.
    {
      ...
      virtual_timer->trigger_time = virtual_timer->client_config.start_time +
                                    virtual_timer->client_config.timeout_period;
      sns_isafe_list_iter_init(&iter, &state->periodic_timers, true);
...
#ifdef SNS_TIMER_ENABLE_DEBUG
      SNS_PRINTF(LOW, sns_fw_printf,
                 "Creating virtual timer timeout %x period %x",
...
/* ------------------------------------------------------------------------------------
 */
static sns_rc sns_timer_sensor_deinit(sns_sensor *const this)
{
  sns_timer_state *state = (sns_timer_state *)this->state->state;
  sns_timer_stop(state->system_timer, NULL);
  sns_timer_delete(state->system_timer);

  sns_osa_thread_sigs_set(state->ro_buffer.thread, SNS_OSA_THREAD_EXIT_SIG);
  sns_osa_thread_delete(state->ro_buffer.thread);
  return SNS_RC_SUCCESS;
}

SNS_SECTION(".text.sns")
static sns_sensor_instance *sns_timer_find_matching_instance(
    sns_sensor *const this, sns_timer_instance_state const *inst_state_to_match)
{
  sns_sensor_instance *existing_inst = NULL;
                             ...
                             sns_request const *new_req, bool remove)
{
  ...
  if(remove)
  {
    sns_sensor_instance *inst = sns_sensor_util_find_instance(
    ...
    if(NULL != inst)
    {
      ...
      if(NULL == inst->cb->get_client_request(
                     inst, sns_timer_sensor_get_sensor_uid(this), true))
      {
    ...
    return inst;
          ...
          sns_timer_get_decoded_req(new_req, &decoded_request,
                                    &decoded_payload))
  {
    sns_rc rc = SNS_RC_SUCCESS;
    sns_sensor_instance *timer_inst = NULL;
    sns_sensor_instance *existing_inst = sns_sensor_util_find_instance(
        this, curr_req, this->sensor_api->get_sensor_uid(this));
    sns_timer_instance_state inst_state_to_match = {};
    sns_timer_instance_state *new_inst_state;
    bool matching_inst_found = false;

    if(decoded_payload.is_periodic &&
       (decoded_payload.timeout_period * sns_get_time_tick_resolution()) <
           TIMER_MIN_INTERVAL_NS)
    {
      return NULL;
    }
    ...
    if(existing_inst)
    {
      // remove curr_req from existing instance
      existing_inst->cb->remove_client_request(existing_inst, curr_req);
      if(NULL ==
         existing_inst->cb->get_client_request(
             existing_inst, sns_timer_sensor_get_sensor_uid(this), true))
      {
        sns_timer_sensor_delete_entry(state, existing_inst, true, true);
        timer_inst =
            sns_timer_find_matching_instance(this, &inst_state_to_match);
        // if matching instance can be found, delete existing inst, otherwise,
        // reuse it
        if(timer_inst && timer_inst != existing_inst)
        {
          this->cb->remove_instance(existing_inst);
          matching_inst_found = true;
        }
      }
    }
    // reuse existing instance
    if(!matching_inst_found && existing_inst &&
       NULL == existing_inst->cb->get_client_request(
                   existing_inst, sns_timer_sensor_get_sensor_uid(this), true))
    {
      timer_inst = existing_inst;
    }
    if(!matching_inst_found && !timer_inst)
    {
      // find instance matches new request
      timer_inst = sns_timer_find_matching_instance(this, &inst_state_to_match);
      if(timer_inst)
      {
        matching_inst_found = true;
      }
    ...
    if(NULL != timer_inst)
    {
      ...
      if(!matching_inst_found)
      {
        sns_request inst_req = {.message_id =
                                    SNS_TIMER_MSGID_SNS_TIMER_SENSOR_CONFIG,
      ...
      if(SNS_RC_SUCCESS == rc)
      {
        ...
        SNS_PRINTF(LOW, sns_fw_printf, "nom req trigger %x actual trigger %x",
                   (uint32_t)(decoded_payload.start_time +
                              decoded_payload.timeout_period),
                   (uint32_t)new_inst_state->trigger_time);

#endif
        /* Do not send SNS_TIMER_MSGID_SNS_TIMER_SENSOR_REG_EVENT event to the
         * clients if client has explicitly requested with disable_response =
         * True
         */
        if(false == decoded_payload.disable_response)
        {
                            ...
                            SNS_TIMER_MSGID_SNS_TIMER_SENSOR_REG_EVENT, NULL))
          {
            SNS_PRINTF(ERROR, sns_fw_printf,
                       "Timer registration event failure");
          }
        }

        if((true == decoded_payload.disable_response) &&
           (true == decoded_payload.is_dry_run))
        {
          SNS_PRINTF(ERROR, sns_fw_printf,
                     "Request %X received by Timer sensor,has set both "
                     "is_dry_run and disable_response to True",
                     curr_req);
        }
      }
    }
    ...
    {
      SNS_PRINTF(ERROR, sns_fw_printf, "Unsupported message_id %d",
                 new_req->message_id);
    }
    return timer_inst;
  }
 ...
 */
SNS_SECTION(".text.sns")
static sns_rc sns_timer_notify_event(sns_sensor *const this)
{
  UNUSED_VAR(this);
  return SNS_RC_SUCCESS;
}

const sns_sensor_api timer_sensor_api SNS_SECTION(".rodata.sns") = {
